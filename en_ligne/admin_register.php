...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($email_err) && empty($password_err) && empty($confirm_password_err)) {
        $sql = "INSERT INTO utilisateurs (type_utilisateur, email, mot_de_passe, statut) VALUES ('admin', :email, :mot_de_passe, 'actif')";
        if ($stmt = $pdo->prepare($sql)) {
            $stmt->bindParam(":email", $param_email, PDO::PARAM_STR);
            $stmt->bindParam(":mot_de_passe", $param_password, PDO::PARAM_STR);
            $param_email = $email;
            $param_password = hash('sha256', $password); // Hash compatible with login
            if ($stmt->execute()) {
...
<head>
    <meta charset="UTF-8">
    <title>Inscription Administrateur - AURA_infini</title>
...
<div class="register-container">
    ...
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" novalidate>
        ...
        <div class="mb-3">
            ...
            <a href="admin_login.php" class="btn btn-link">Connexion</a>
        </div>
    </form>
</div>
<!-- Bootstrap JS CDN -->
